function cute_snr_photons,strct
  close,/all
  ;Constants
  ...
  if (tag_exist(strct,'flux_file') eq 1)then begin ; if the stellar flux at earth is provided as a file of wavelength vs flux
    ...
    lca_strct={magnitude:V_mag,radius:r_star,temperature:t_star,Ra:ra,Dec:dec,stype:sptype,logr:strct.logr}
    ...
    if strct.line_core_emission eq 1 then data=cute_snr_lca(flux,lca_strct,0.257,0.288) else data= flux ;line core emission
    wave=data[0,*]
    ...
    if strct.add_ism_abs eq 1 then flux_data=cute_ism_abs_all(data,nmg2,nmg1,nfe2) else flux_data=data;use for adding ISM of Mg2, Mg1 and Fe2
    flux_di=flux_data[1,*]
